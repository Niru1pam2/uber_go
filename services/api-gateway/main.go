...
func main() {
	...
	mux.Handle("POST /trip/preview", tracing.WrapHandlerFunc(enableCORS(handlePreviewTrip), "/trip/preview"))
	...
	case sig := <-shutdown:
		...
		if err := server.Shutdown(ctx); err != nil {
			log.Print("Could not stop the server gracefully", err)
			server.Close()
		}
	}
